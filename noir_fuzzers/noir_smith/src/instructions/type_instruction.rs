...
fn type_operation_rec(
    ...
    let element1: Operand = if depth == 0 || random.gen_bool() {
        if random.gen_bool() {
            ...
            match var {
                ...
                None => Operand::Value(
                    value::random_value(random, chosen_type),
                    chosen_type.clone(),
                ),
            }
        } else {
        ...
        Operand::Operation(Box::new(type_operation_rec(
            random,
            bloc_data,
            chosen_type,
            depth - 1,
    ...
    Operation::new(
        chosen_type,
        random.choose_random_item_from_vec(&var_type::supported_arithmetic_operator_by_type(
            chosen_type,
        )),
        element1,
        element2,
...
pub fn generate_type_instruction(
    random: &mut Random,
    bloc_data: &BlocData,
    list_global: &BlocData,
    list_functions: &ListFunctions,
    list_structs: &ListStructs,
    instruction_type: &VarType,
    depth: usize,
) -> String {
    if depth == 0 {
        match random.gen_range(0, 2) {
            0 => {
                match bloc_data.get_random_variable(
                    random,
                    vec![&instruction_type],
                    matches!(instruction_type, VarType::Reference(_)),
                ) {
        ...
        match random.gen_range(0, 5) {
            ...
            1 => {
                if bloc_data.is_variables_empty()
                    | var_type::supported_arithmetic_operator_by_type(instruction_type).is_empty()
                {
            ...
            2 => {
                ...
                match list_functions.call_by_type(
                    random,
                    bloc_data,
                    list_global,
                    list_structs,
            ...
            3 => {
                ...
                if let Some(var) = bloc_data.get_random_variable(
                    random,
                    vec![&struct_type],
                    matches!(instruction_type, VarType::Reference(_)),
                ) {
                    let name_and_way = var.name_and_way(random, &struct_type);
                    if let Some(s) = random_struct.call_by_type(
                        random,
                        bloc_data,
                        list_global,
                        list_functions,
            ...
            4 => {
                ...
                match bloc_data.get_random_lambda(random, vec![instruction_type.clone()]) {
                    Some(l) => {
                        ...
                        let way = var_type::way_to_type(
                            random,
                            &l.ret_type().clone().expect("lambda with no ret type"),
                            instruction_type,
                            &mut is_a_ref,
                        )
                        .expect("no way to type");
