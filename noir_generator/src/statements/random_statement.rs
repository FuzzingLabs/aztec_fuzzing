...
pub fn generate_random_statement(random: &mut Random, bloc_variables: &mut BlocData, list_global: &BlocData, list_functions: &ListFunctions, list_structs: &ListStructs) -> String {
    ...
    if random.gen_range(0, 10) == 0 {
        return declaration_statement::generate_lambda_declaration(random, bloc_variables, list_global, list_functions, list_structs)
    }

    match random.gen_range(0, 2) {
        0 => declaration_statement::generate_variable_declaration(random, bloc_variables, list_global, list_functions, list_structs),
        1 => operation_statement::generate_operation_instruction(random, bloc_variables, list_global, list_functions, list_structs),
