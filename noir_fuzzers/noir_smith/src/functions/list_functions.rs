...
use crate::{
    ...
    variables::{
        ...
        var_type::{self, is_same_type, VarType},
        variable::Variable,
    },
};
...
impl ListFunctions {
    ...
    pub fn call_by_type(
        ...
        let valid_function: Vec<&Function> = self
            .functions
            .iter()
            .filter(|&e| {
        ...
        if is_main {
            for _ in 0..CONFIG.max_function_arguments {
                bloc_variables.add_variable(Variable::new(
                    ...
                    false,
                    &var_type::random_basic_type(random),
                ));
            }
            ...
            for _ in 0..CONFIG.max_function_arguments {
                ...
                if rand == 0 {
                    ...
                    bloc_variables.add_variable(Variable::new(
                        bloc_variables.next_variable_name(),
                        false,
                        &VarType::Generic(vec_trait.clone()),
                    ));
                } else if rand == 2 {
                    bloc_variables.add_variable(Variable::new(
                        bloc_variables.next_variable_name(),
                        false,
                        &VarType::Array(
                            Box::new(VarType::Generic(vec_trait.clone())),
                            usize::max_value(),
                        ),
                    ));
                } else {
