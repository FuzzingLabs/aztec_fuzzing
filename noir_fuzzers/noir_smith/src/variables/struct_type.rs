use crate::{
    functions::{list_functions::ListFunctions, method::Method},
    tools::constants::CONFIG,
    tools::random::Random,
    variables::{var_type, variable::Variable},
};

use super::{
    bloc_data::BlocData,
    list_structs::ListStructs,
    var_type::{is_same_type, VarType},
};
...
impl StructType {
    ...
    pub fn generate_struct_code(
        ...
        format!(
            "{}{}",
            struct_string,
            self.generate_impl_code(random, list_global, list_functions, list_structs)
        )
    }

    /// Return a string representing the code required for the implementation part of this structure
    pub fn generate_impl_code(
        &mut self,
        random: &mut Random,
        list_global: &BlocData,
        list_functions: &ListFunctions,
        ...
        for i in 0..random.gen_range(0, CONFIG.max_method_by_struct) {
            ...
            impl_string = format!(
                "{}\n{}\n",
                impl_string,
                new_method.initialize(random, list_global, list_functions, list_structs, self)
            );
    ...
    pub fn call_by_type(
        ...
        let valid_method: Vec<&Method> = self
            .methods
            .iter()
            .filter(|&e| {
        ...
        Some(random.choose_random_item_from_vec(&valid_method).call(
            random,
            bloc_variables,
            list_global,
            list_functions,
