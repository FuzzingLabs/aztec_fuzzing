...
pub fn random_type_without_reference(random: &mut Random, list_structs: &ListStructs) -> VarType {
    match random.gen_range(0, 7 + CONFIG.use_of_slice) {
        ...
        3 => VarType::bool,
        4 => VarType::str(random.gen_range(0, CONFIG.max_composite_size)),
        5 => VarType::array(Box::new(random_type_without_reference_with_depth(random, list_structs, CONFIG.max_composite_depth)), random.gen_range(0, CONFIG.max_composite_size)),
        6 => {
            let size = random.gen_range(2, CONFIG.max_composite_size);
            let mut vec_tup = Vec::with_capacity(size);
            for _ in 0..size {
                vec_tup.push(Box::new(random_type_without_reference_with_depth(random, list_structs, CONFIG.max_composite_depth)));
            }
            VarType::tup(vec_tup)
        },
        7 => VarType::slice(Box::new(random_type_without_reference_with_depth(random, list_structs, CONFIG.max_composite_depth)), random.gen_range(0, CONFIG.max_composite_size)),
        _ => VarType::field,
    }
}

/// Used to limit the depth of composite types randomly generated
pub fn random_type_without_reference_with_depth(random: &mut Random, list_structs: &ListStructs, depth: usize) -> VarType {
    if depth == 0 {
        match random.gen_range(0, 5) {
            0 => VarType::field,
            1 => VarType::uint(random_bit_size(random)),
        ...
        match random.gen_range(0, 7) {
            ...
            3 => VarType::bool,
            4 => VarType::str(random.gen_range(0, CONFIG.max_composite_size)),
            5 => VarType::array(Box::new(random_type_without_reference_with_depth(random, list_structs, depth -1)), random.gen_range(0, CONFIG.max_composite_size)),
            6 => {
                let size = random.gen_range(2, CONFIG.max_composite_size);
                let mut vec_tup = Vec::with_capacity(size);
                for _ in 0..size {
                    vec_tup.push(Box::new(random_type_without_reference_with_depth(random, list_structs, depth -1)));
                }
                VarType::tup(vec_tup)
            },
            _ => VarType::field,
...
pub fn supported_arithmetic_operator_by_type(var_type: &VarType) -> Vec<Operator> {
    match var_type {
        ...
        VarType::int(_) => vec![Operator::Add, Operator::Subtract, Operator::Multiply, Operator::Divide], //Operator::Lshift, Operator::Rshift
        VarType::uint(_) => vec![Operator::Add, Operator::Subtract, Operator::Multiply, Operator::Divide], //Operator::Lshift, Operator::Rshift
