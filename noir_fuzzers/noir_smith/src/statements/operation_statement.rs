...
pub fn generate_operation_instruction(
    random: &mut Random,
    ...
    let instr_string = generate_type_instruction(
        random,
        bloc_variables,
        list_global,
        list_functions,
    ...
    match bloc_variables.get_random_variable(random, vec![&chosen_type], true) {
        Some(assigned_var) => {
            instruction = format!(
                "{}{} = ",
                instruction,
                assigned_var.name_and_way(random, &chosen_type)
            )
        }
        None => {
            let assigned_var = Variable::new(
