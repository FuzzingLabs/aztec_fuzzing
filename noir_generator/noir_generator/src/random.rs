...
pub fn generate_random_name() -> String {
    let mut rng = RNG.lock().unwrap();
    let name_length = rng.rng.gen_range(5..15);
    let mut name = String::with_capacity(name_length);

    for _ in 0..name_length {
        let random_char = rng.rng.gen_range(b'a'..=b'z') as char;
...
pub fn generate_random_value_for_basic_type(type_: &'static str) -> String {
    let mut rng = RNG.lock().unwrap();
    match type_{
...
pub fn generate_random_value_for_composite_type(type_: &'static str) -> String {
    let mut rng = RNG.lock().unwrap();
    match type_{
        "Field" => rng.rng.gen_range(0..=u128::MAX).to_string(),
        "u8" => rng.rng.gen_range(0..=u8::MAX).to_string(),
