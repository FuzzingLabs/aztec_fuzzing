use crate::{
    instructions::{
        comparison_instruction::generate_comparison_instruction,
        type_instruction::generate_type_instruction,
    },
    statements::random_statement,
    tools::constants::CONFIG,
    tools::random::Random,
    variables::{
        bloc_data::BlocData,
...
impl Function {
    ...
    pub fn call(
        ...
        for i in 0..vars.len() {
            match vars[i].var_type() {
                VarType::Array(v_type, size) => {
                    if var_type::is_same_type(v_type, &VarType::Generic(Vec::new()))
                        || *size == usize::max_value()
                    {
                        let type_to_set =
                            if var_type::is_same_type(v_type, &VarType::Generic(Vec::new())) {
                                Box::new(generic_type.clone())
                            } else {
                                v_type.clone()
                            };
                        vars[i].set_type(&VarType::Array(type_to_set, composite_generic_size));
                    }
                }
        ...
        if lambdas.len() != 0 {
            for i in 0..lambdas.len() - 1 {
                call = format!(
                    "{}{}, ",
                    call,
                    lambdas[i].initialize_as_argument(
                        random,
                        list_global,
                        list_functions,
                        list_structs
            ...
            call = format!(
                "{}{}",
                call,
                lambdas[lambdas.len() - 1].initialize_as_argument(
                    random,
                    list_global,
                    list_functions,
                    list_structs
            ...
            Some(v) => match v {
                VarType::Tup(_) => format!(
                    ...
                    generate_type_instruction(
                        ...
                        list_structs,
                        v,
                        CONFIG.max_instruction_depth
                    )
                ),
                _ => {
                    let mut ret = generate_type_instruction(
                        random,
                        bloc_variables,
                        list_global,
                        list_functions,
    ...
    pub fn generate_function_code(
            ...
            match var.var_type() {
                VarType::Array(v_type, _) => {
                    if !var_type::is_same_type(v_type, &VarType::Generic(Vec::new())) {
                        ...
                        name_number_to_skip += 1;
                    }
                }
                VarType::Generic(_) => name_number_to_skip += 1,
                _ => bloc_variables.add_variable(var),
            }
        }
        bloc_variables.set_name_number_to_skip(name_number_to_skip);
        ...
        let mut nb_instructions_left: usize =
            random.gen_range(0, CONFIG.max_instruction_by_function);
    ...
    pub fn generate_else(
        ...
        list_functions: &ListFunctions,
        list_structs: &ListStructs,
        fun_bloc_variables: &BlocData,
        mut nb_instructions_left: usize,
    ) -> String {
    ...
    pub fn generate_if(
        ...
        let mut function_string: String = format!(
            "if {}{{\n",
    ...
    pub fn generate_for(
        ...
        let mut function_string: String =
